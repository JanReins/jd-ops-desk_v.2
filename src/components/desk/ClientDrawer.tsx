...
import { generateNextMonthCandidates, CandidateObligation } from '@/lib/ops/obligationGenerator';
import { useData } from '@/lib/ops/data-context';
import { Link } from '@tanstack/react-router';
import { METKA_ENTITIES, isMetkaClient, summarizeMetkaPack } from '@/lib/ops/metka';
      ...
      >
        ...
        <div className="border-b border-slate-200 px-6 py-4 bg-slate-50 shrink-0">
          ...
          <div className="grid grid-cols-3 gap-2 mt-3 pt-3 border-t border-slate-200 text-xs">
            <div>
              <span className="text-slate-400 text-2xs uppercase font-bold tracking-wider">Junior</span>
              <p className="font-semibold text-slate-800">{client.junior || 'Unassigned'}</p>
            </div>
            <div>
              <span className="text-slate-400 text-2xs uppercase font-bold tracking-wider">Senior</span>
              <p className="font-semibold text-slate-800">{client.senior || 'Unassigned'}</p>
            </div>
            <div>
              <span className="text-slate-400 text-2xs uppercase font-bold tracking-wider">Manager</span>
        ...
        <div className="flex-1 overflow-y-auto p-6 space-y-6 text-xs">
              ...
              {(Object.keys(SERVICE_LABELS) as (keyof ClientServices)[]).map((key) => {
                ...
                return (
                  ...
                  >
                    ...
                    <span
                      className={`h-4 w-4 rounded flex items-center justify-center shrink-0 text-2xs ${
                        isActive
          ...
          {isMetkaClient(client.id) && (
            <div className="rounded-lg border border-line bg-paper p-4">
              <div className="mb-2 flex items-center justify-between gap-2">
                <h3 className="text-xs font-bold uppercase tracking-wider text-ink">Entity pack</h3>
                <Link
                  to="/trackers"
              ...
              <p className="mb-2 text-2xs text-muted">
                {METKA_ENTITIES.length} legal entities. Monthly BAS is one cell each — not {METKA_ENTITIES.length} clients.
                {(() => {
                  const pack = summarizeMetkaPack(obligations, getMelbourneCurrentPeriod());
                  return pack.total
                    ? ` This month: ${pack.done}/${METKA_ENTITIES.length} done · ${pack.open} open.`
                    : " This month pack opens from Trackers.";
                })()}
              </p>
              <ul className="max-h-48 space-y-1 overflow-y-auto">
                {METKA_ENTITIES.map((entity) => (
                  <li key={entity.name} className="flex items-baseline justify-between gap-2 text-2xs">
                    <span className="min-w-0 truncate font-medium text-ink" title={entity.name}>
                      {entity.shortName}
                      <span className="font-normal text-subtle">
                        {entity.group ? " · Group" : ""}
                      </span>
                    </span>
                    <span className="shrink-0 text-subtle">{entity.tax}</span>
                  </li>
                ))}
              </ul>
            </div>
                  ...
                  >
                    <div className="flex items-start justify-between gap-2">
                      <div className="flex items-center gap-1.5 flex-wrap">
                        ...
                        {ob.carryOver && (
                          <span className="text-2xs font-semibold px-1 rounded bg-amber-100 text-amber-800">
                            Carry
                          </span>
                        )}
                      </div>
                    ...
                    {ob.nextAction && (
                      <div className="mt-1.5 text-2xs text-slate-600 bg-slate-50 px-2 py-1 rounded border border-slate-100 truncate">
                        <strong className="text-slate-700">Next:</strong> {ob.nextAction}
                      </div>
                    )}

                    {ob.blocker && (
                      <div className="mt-1 text-2xs text-rose-700 bg-rose-50 px-2 py-0.5 rounded border border-rose-200">
                ...
                <div className="space-y-1.5">
                  {completedObligations.map((ob) => (
                    ...
                    >
                      ...
                      <span className="text-emerald-700 text-2xs font-semibold shrink-0">
                        Done ✓
                      </span>
                    </div>
                  ))}
