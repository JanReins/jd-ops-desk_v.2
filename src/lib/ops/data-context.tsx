...
export function DataProvider({ children }: { children: ReactNode }) {
  ...
  const updateClient = async (id: string, updates: Partial<Client>) => {
    ...
    if (!target) return;
    const now = new Date().toISOString();
    const updatedClient: Client = { ...target, ...updates, id: target.id, userId: UID, updatedAt: now };
  ...
  const addClient = async (
    ...
    const now = new Date().toISOString();
    await putClient({ ...data, id: newId, userId: UID, createdAt: now, updatedAt: now });
    await refreshData();
    return newId;
  };
  ...
  const reorderTodayPlan = async (orderedIds: string[]) => {
    ...
    orderedIds.forEach((id, index) => {
      const target = obligations.find((o) => o.id === id);
      if (!target) return;
      updates.push({ ...target, todayOrder: index + 1, order: index + 1, updatedAt: now });
    });
